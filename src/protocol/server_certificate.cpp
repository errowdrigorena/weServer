...
void load_server_certificate(boost::asio::ssl::context &ctx) {
    auto const cert = read_file_content(std::filesystem::current_path() / "server.cert.pem");
    auto const key = read_file_content(std::filesystem::current_path() / "server.key.pem");
