...
ResponseBuilder::ResponseBuilder(unsigned version, bool keep_alive)
...
ResponseBuilder::MessageGenerator ResponseBuilder::plaintext_response(
    boost::beast::http::status status,
    std::string content)
{
    header_.result(status);
    boost::beast::http::response<boost::beast::http::string_body> response{
        move_header(), std::move(content)};
...
RequestContext::RequestContext(Request&& request)
    ...
    , response_{request_.version(), request_.keep_alive()}
{
}
