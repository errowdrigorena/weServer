...
read_file_content(const fs::path& filename)
{
    ...
    std::string returnable{std::istreambuf_iterator<char>(file), std::istreambuf_iterator<char>() };
    return returnable;
}
...
load_server_certificate(boost::asio::ssl::context& ctx)
{
    ...
    */
    ...
    std::string const cert = read_file_content(fs::current_path()/"server.cert.pem");
    ...
    std::string const key = read_file_content(fs::current_path()/"server.key.pem");
